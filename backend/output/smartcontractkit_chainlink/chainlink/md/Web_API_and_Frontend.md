...
This page documents the web API and frontend components of the Chainlink node. It covers the HTTP server, API endpoints, authentication mechanisms, GraphQL interface, and the operator UI. If you're looking for information about specific API integrations like bridges or external initiators, see [Bridge and External Initiator System]().

## Overview

The Chainlink node provides a web interface consisting of two main components:

1. **REST API**: A HTTP server exposing endpoints for node administration, job management, and blockchain interaction
2. **Operator UI**: A web-based interface for node operators to monitor and manage node operations
